...
void vec2::normalize(const float r)
{
...
vec2 operator * (const float c, const vec2& v)
{
    return v * c;
}
...
vec3::vec3(const vec2& v, const float s)
{
    x = v.x, y = v.y, z = s;
}

// Vector normalization
void vec3::normalize(const float r) {
...
{
    float val = u.x * v.x + u.y * v.y + u.z * v.z;
...
vec4::vec4(float const a, float const b, float const c, float const d)
{
    x = a, y = b, z = c, w = d;
